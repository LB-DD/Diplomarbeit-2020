...
//Testscript for lokal Ganache Testnet

var Web3 = require('web3')
var masterDataContractJson = require('../truffle/build/contracts/MasterData.json')
var testAccs = parseAccs(require('../../testAccounts/ganacheTestAccounts.json'))
var web3 = undefined
...
async function initWeb3() {
  return new Promise(async (resolve, reject) => {
    ...
    const provider = new Web3.providers.WebsocketProvider(
      'ws://localhost:8545',//classic local port for testing,
      {
        clientConfig: {
          keepalive: true,
...
async function loadOrgElementDatakeys(id) {
  ...
  try {
    ...
    console.log("error when laoding data keys", err)
  }
}
...
function parseAccs(testJson) {
  let testAccs = []
  for (let prop in testJson.private_keys) {
    testAccs.push([prop, '0x' + testJson.private_keys[prop]])
  }
  return testAccs
}